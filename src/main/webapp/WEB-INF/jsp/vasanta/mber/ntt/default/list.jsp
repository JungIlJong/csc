  ...
  <main class="customer-main">
    <div class="page-tit">
      <div class="page-tit-wrap">
      	<c:set var="menuNm" value="${bbsId eq '15' ? 'MOU체결' : ''}"></c:set>
        <h1><c:out value="${not empty menuTitle ? menuTitle : menuNm}"/></h1>
        <p><c:out value="${menuSj}"/></p>
      </div>
      <ul class="rocation">
        <c:forEach var="m" items="${breadcrumb}" varStatus="status">
		    <li>
		      <c:choose>
		        <c:when test="${status.last}">
		          <span><c:out value="${m.menuNm}" /></span>
		        </c:when>
		        <c:otherwise>
